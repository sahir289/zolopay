...
import { Columns, EditRowFormFields, PayInStatusOptions, resetPayInFormFields, Role, Status } from "@/constants";
...
import { resetPagination, setPagination } from "@/redux-toolkit/slices/common/params/paramsSlice";
import { getAllMerchantCodes } from "@/redux-toolkit/slices/merchants/merchantAPI";
import { checkPendingStatus, getAllPayIns, processUtr, updatePayIns, updatePayInsData } from "@/redux-toolkit/slices/payin/payinAPI";
import { getAllPayInData, getIsloadingPayinEntries, getRefreshPayIn } from "@/redux-toolkit/slices/payin/payinSelectors";
import { clearPayInFilter, getPayIns, onload, setIsloadingPayinEntries, setPayInFilter, setRefreshPayIn } from "@/redux-toolkit/slices/payin/payinSlice";
import { getAllVendorCodes } from "@/redux-toolkit/slices/vendor/vendorAPI";
import { ArrowDownWideNarrow, ChevronDown, Download, HardDrive, History, RefreshCw, Search, Timer, X } from "lucide-react";
...
export default function Payins ({status}: PayinsProps) {
    ...
    const shouldSetSearchInAddData = (
        ...
        const hasFilterStateValues = Boolean(
          ...
          (typeof filters.user_ids !== 'boolean' && filters.user_ids?.length) || 
          filters.status || 
          filters.updated_at ||
    ...
    const getPayin_Merchant_Columns = () => {
        switch (status) {
            case 'all':
                return Columns.PAYIN_MERCHANT;
            case 'completed':
    ...
    const getPayin_Vendor_Columns = () => {
        switch (status) {
            case 'all':
                return Columns.PAYIN_VENDOR;
            case 'completed':
                                            ...
                                            >
                                                ...
                                                <div className="mt-3">
                                                    ...
                                                    {[
                                                        ...
                                                        ? getPayin_Merchant_Columns()
                                                        : role &&
                                                            [
                                                            Role.VENDOR,
                                                            Role.VENDOR_OPERATIONS,
                                                            ].includes(role)
                                                        ? getPayin_Vendor_Columns()
                                                        : getPayin_Columns()),
                        ...
                        <div className="overflow-auto xl:overflow-visible">
                            {payins.loading && payins.isloadingPayinEntries ? (
                                ...
                                <CustomTable
                                    ...
                                    setImgUtr={(status === 'completed' || status === 'dropped') ? undefined : setImgUtr}
                                    currentPage={Number(pagination?.page) || 1}
                                    pageSize={Number(pagination?.limit) || 20}
                                    onPageChange={handlePageChange}
                                    onPageSizeChange={handlePageSizeChange}
                                    expandedRow={status === 'completed' ? expandedRow : undefined}
                                    handleRowClick={status === 'completed' ? handleRowClick : undefined}
                                    actionMenuItems={(row: any) => {
                                        ...
                                        if (row?.status === Status.SUCCESS && (status === 'all' || status === 'completed')) {
                                        if (
        ...
        {newTransactionModal && (
            ...
            >
                <DynamicForm
                    sections={
                        payInData.status === Status.BANK_MISMATCH
                        ? resetPayInFormFields(bankNames).RESET_BANK
            ...
            >
                ...
                <div>
                    ...
                    <DynamicForm
                        ...
                        sections={EditRowFormFields(bankNames, {
